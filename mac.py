...
check = 0
if os.path.isfile(path + '/chromedriver') == True : 
    check = 1
    print(path)
...
class LoginWindow(QMainWindow, ui):
    def __init__(self):
        ...
        if check == 0 :
            ...
            doc = urllib.request.urlopen('https://chromedriver.storage.googleapis.com', context=context).read()
    ...
    def login(self) :
        ...
        if self.login_id.text() == "" : # school id is blank
            ...
            path = str((os.path.sep.join(sys.argv[0].split(os.path.sep)[:-1]))) + '/chromedriver'
            driver = webdriver.Chrome(executable_path=path, service_args=args, chrome_options=options)
...
ui_main_path = resource_path("main.ui")
...
class MainWindow(QMainWindow, ui_main):
    def __init__(self):
        ...
        # Logo
        self.logo_label.setPixmap(QPixmap(resource_path('logo.jpg')))
...
ui_message_path = resource_path("message.ui")
...
ui_assign_path = resource_path("assign.ui")
...
ui_video_chat_path = resource_path("video_chat.ui")
...
ui_select_class_path = resource_path("select_class.ui")
